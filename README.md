...
to the backend. When an upload is approved the webhook handler now calls
Cloudinary's `explicit` API to generate the overlay and thumbnail derivatives so
they appear alongside the original asset.
